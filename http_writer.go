...
func NewHTTPWriter(c HTTPWriterConfig, consistency string) *HTTPWriter {
	return &HTTPWriter{
		...
		c:   c,
		url: []byte(c.Host + "/api/v2/write?org=" + c.InfluxOrg + "&bucket=" + url.QueryEscape(c.Database)),
...
func (w *HTTPWriter) WriteLineProtocol(body []byte, isGzip bool) (int64, error) {
	...
	//log.Printf("TEST=%s",w.c.InfluxToken)
	//s := "Token " + "zqwOzLlS0r46jBxrl0njmdXsf7vK-pHeha2_-YMyBGg3JBUmv32TLwsXXhcZu5LuELYMmTb0jkhT9Q3YSrPPbA=="
	s := "Token " + w.token
	//test adding token to header
