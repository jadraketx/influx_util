...
func (l *InfluxBulkLoad) Init() {
	...
	flag.StringVar(&l.influxToken, "token", "empty", "InfluxDB 2.0 token")
	flag.StringVar(&l.influxOrg, "org", "TACC", "InfluxDB 2.0 user organization (default TACC)")
...
func (l *InfluxBulkLoad) PrepareProcess(i int) {
	...
	l.configs[i].writer = NewHTTPWriter(HTTPWriterConfig{
		...
		InfluxToken:    l.influxToken,
		InfluxOrg:      l.influxOrg,
...
outer:
	for scanner.Scan() {
		...
		//Handle situation where output is from influx write dryrun - for some reason they include slashes
		if line[0:1] == "\\" {
		...
		//fmt.Printf("Total points: %d\ttotal values:%d\n",totalPoints,totalValues)
	...
	log.Printf("Items read: %d\tPoints read:%d\n", l.itemsRead, l.valuesRead)
		...
		if bulk_load.Runner.DoLoad {
			...
			for {
				if l.useGzip {
					...
					_, _ = fasthttp.WriteGzip(compressedBatch, batch.Buffer.Bytes())
